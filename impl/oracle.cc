...
void KOracle::load_oracle(const string& file, bool is_training) {
  ...
  while(getline(in, line)) {
    ...
    cur_acts.clear();
    if (line.size() == 0 || (line[0] == '!' && line[3] == '(')) continue;//MODIFICADO:if (line.size() == 0 || (line[0] == '#' && line[2] == '(')) continue;
    ...
    while(getline(in, line)) {
      ...
      if (line == kREDUCE) {
        ...
        // PJ(X) is put into the actions list as PJ(X). Lo cambiamos por NT otra vez
